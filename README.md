...
meow/dataset 数据集
meow/data_analysis 数据处理——>造feature

meow/model 模型定义文件——>对比模型
...
meow/model/lstm.py 为LSTM网络建立文件，可以在这里修改网络结构

meow/model/lstm_attention.py 为LSTM_Attention网络建立文件，可以在这里修改网络结构

meow/model/MTSF-DG为图神经网络模型，尝试复现，未成功......

meow/mdl.py为Ridge线性模型

meow/mdl_lstm.py为LSTM模型

meow/mdl_lstm_attention.py为改进的LSTM_Attention模型

meow/mdl_XGBoost.py为XGBoost模型【我们对比实验中取得了SOTA】

meow/mdl_tabnet.py为tabnet模型【新调研的模型——>尝试修改 未成功】

`python meow.py`训练Ridge线性回归模型 并进行推理

`python meow_lstm.py`训练LSTM模型 并进行推理

`python meow_lstm_attention.py`训练LSTM-Attention模型 并进行推理

`python meow_XGBoost.py`训练XGBoost决策树模型 并进行推理

xgboost_model.json为存储的XGBoost模型的权重信息，可以直接读取进行推理

*.pth为LSTM网络的权重文件——>注意：best_model.pth为78feature的，best_LSTM_features72.pth为72feature的

## 结果记录

hjb进行了模型测试和结果记录，将几个比较好的结果记录如下：
...
==**final_model!**（虽然只是调库，但是确实效果最好）==XGBoost(78 features(去掉时间和数量基础特征), 'n_estimators': 100,
            'booster': 'gbtree',
            'objective': 'reg:squarederror',
            'max_depth': 7,
            'lambda': 3,
